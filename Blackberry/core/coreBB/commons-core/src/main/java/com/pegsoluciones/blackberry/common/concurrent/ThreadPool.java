...
public class ThreadPool {
	 ...
	 * for the thread pool.
	 */
	Vector assignments = new Vector(3);
	...
	public synchronized Runnable getAssignment() {
		...
		try {
			while ( !assignments.elements().hasMoreElements() )
				wait();
			
			Runnable r = (Runnable) assignments.elements().nextElement();
			assignments.removeElement(r);
			return r;
	...
	protected void finalize() {
		...
		for (int i=0;i<threads.length;i++) {
			...
			// FIXME I changed this line nulling the thread
			// intead of calling .destoy()
			threads[i] = null;
		}
		done.waitDone();
	}
